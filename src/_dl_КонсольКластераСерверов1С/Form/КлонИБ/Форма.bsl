...
Процедура ЗаполнитьПараметры()
	...
	Если ТекущееШаблонРазвертки = "ВсеНаОдном" Тогда
		...
		ЗначенияПоУмолчанию = ПолучитьЗначенияПоУмолчаниюИзМакета("App");
	...
	ИначеЕсли ТекущееШаблонРазвертки = "Кластер" Тогда		
		...
		ЗаписатьЭлементыВДерево(ДеревоСерверов, Истина); 
		ЗаписатьРеквизитыВРС(ОбщиеVM_AD, 		ЗначенияПоУмолчанию, "VM");
		ЗаписатьРеквизитыВРС(ОбщиеVM_endpoint, 	ЗначенияПоУмолчанию, "VM");
		ЗаписатьРеквизитыВРС(ОбщиеVM_vSphere, 	ЗначенияПоУмолчанию, "VM");
		ЗаписатьРеквизитыВРС(ОбщиеVM_advanced, 	ЗначенияПоУмолчанию, "VM");
		...
		ЗаписатьЭлементыВДерево(ДеревоСерверов, Истина);
		ЗаписатьРеквизитыВРС(ОбщиеSQL_advanced, ЗначенияПоУмолчанию, "SQL");
		...
		ЗаписатьЭлементыВДерево(ДеревоСерверов, Истина); 
		ЗаписатьРеквизитыВРС(Общие1С_advanced, ЗначенияПоУмолчанию, "Кластер1С"); 
		...
		Если ТаблицаИменIpИИмен.Количество() = 5 Тогда
		...
		Для каждого ГруппировкаЭлементов Из МассивГруппировокЭлементов Цикл			
			...
			Если ГруппировкаЭлементов = "Application servers" Тогда
				МассивИменЭлементов.Добавить(ИмяНоды1);
				МассивИменЭлементов.Добавить(ИмяНоды2); 
			...
			ИначеЕсли ГруппировкаЭлементов = "Monitor nodes" Тогда 
				МассивИменЭлементов.Добавить(ИмяНоды3);
			...
			ИначеЕсли ГруппировкаЭлементов = "Data nodes" Тогда
				МассивИменЭлементов.Добавить(ИмяНоды4);
				МассивИменЭлементов.Добавить(ИмяНоды5); 
...
Процедура ДеревоРеквизитовЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.ТипЗначения = "СписокЗначений" Тогда
		...
		ОткрытьФорму("ВнешняяОбработка.Форма.РедактированиеСписка", ПараметрыФормы, Элемент,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);	
	КонецЕсли;
...
Процедура ЗаполнитьДанныеПоУмолчанию()
	...
	ДанныеAPP 		= ПолучитьЗначенияПоУмолчаниюИзМакета("App");
	...
	//Домен
	СтруктураПоУмолчанию.Вставить("domain", 		ДоменКомпании);
	...
	//listen_addresses //Из диапазона
	//linux_cluster_ip //из диапазона
	//monitor_node  // из диапазона имен
	//db_dump_path
	////VM
	//hostname_vm //Из диапазона 
	//host_ip    //Из диапазона
	//host_vmware_vlan //Из диапазона
	//
	//ansible_user   //из VM общие
	//ansible_ssh_pass
	//ansible_become_password
	//
	////SQL	
	//host_ip  //Из диапазона
	//host_name  //Из диапазона
	//
	//linux_cluster_name //собираем по шаблону   Собираем по шаблону digilabs-clone-prod_cmdb01 
	//
	//ones_base_name //база ИБ - База данных в кластере
	//db_name  // База данных из связанной КЕ Database 1C     
	//
	//postgres_cluster_name //из sql
	//postgres_pass 
	//
	//node_name //=Имя ноды + “.” + Домен компании
	//central_server //первая нода
