...
Процедура ЗаполнитьПараметры()
	...
	Если ТекущееШаблонРазвертки = "ВсеНаОдном" Тогда
		...
		СтрокаДереваСерверов.ГруппировкаСерверов	= ПредопределенноеЗначение("Перечисление._dl_ГруппировкиСерверовСозданияИнфраструктуры.ApplicationServers");
		
		Для каждого СтрокаТЧ Из РеквизитыДиска Цикл
			...
			Запись.ГруппировкаСерверов		= ПредопределенноеЗначение("Перечисление._dl_ГруппировкиСерверовСозданияИнфраструктуры.ApplicationServers");
	...
	ИначеЕсли ТекущееШаблонРазвертки = "Кластер" Тогда		
		...
		СтрокаРодитель.ГруппировкаСерверов		= ПредопределенноеЗначение("Перечисление._dl_ГруппировкиСерверовСозданияИнфраструктуры.ApplicationServers");
		
		ТекущийИдентификаторЭлемента 	= Новый УникальныйИдентификатор;
		ТекущееИмяЭлемента 				= "App server №1";
		...
		СтрокаДерева.ГруппировкаСерверов		= ПредопределенноеЗначение("Перечисление._dl_ГруппировкиСерверовСозданияИнфраструктуры.ApplicationServers");
		...
		ТекущийИдентификаторЭлемента 	= Новый УникальныйИдентификатор;
		ТекущееИмяЭлемента 				= "App server №2";
		...
		СтрокаРодитель.ИмяЭлемента          = ТекущееИмяЭлемента; 
		СтрокаРодитель.Редактируемый		= Истина; 
		СтрокаРодитель.ГруппировкаСерверов		= ПредопределенноеЗначение("Перечисление._dl_ГруппировкиСерверовСозданияИнфраструктуры.MonitorNodes");
		...
		Для каждого СтрокаТЧ Из РеквизитыДиска Цикл
			...
			Запись.ИмяЭлемента 				= ТекущееИмяЭлемента;
			Запись.ИдентификаторЭлемента 	= ТекущийИдентификаторЭлемента;
			Запись.ГруппировкаСерверов		= ПредопределенноеЗначение("Перечисление._dl_ГруппировкиСерверовСозданияИнфраструктуры.MonitorNodes");
			Запись.МестоИспользования		= "КластерСерверов";
		...
		СтрокаРодитель.ИмяЭлемента          = ТекущееИмяЭлемента; 
		СтрокаРодитель.Редактируемый		= Истина; 
		СтрокаРодитель.ГруппировкаСерверов		= ПредопределенноеЗначение("Перечисление._dl_ГруппировкиСерверовСозданияИнфраструктуры.DataNodes");
		...
		СтрокаДерева.ИдентификаторЭлемента 	= ТекущийИдентификаторЭлемента;
		СтрокаДерева.Редактируемый			= Ложь;
		СтрокаДерева.ГруппировкаСерверов		= ПредопределенноеЗначение("Перечисление._dl_ГруппировкиСерверовСозданияИнфраструктуры.DataNodes");
		...
		Для каждого СтрокаТЧ Из РеквизитыДиска Цикл
			...
			Запись.ИмяЭлемента 				= ТекущееИмяЭлемента;
			Запись.ИдентификаторЭлемента 	= ТекущийИдентификаторЭлемента;
			Запись.ГруппировкаСерверов		= ПредопределенноеЗначение("Перечисление._dl_ГруппировкиСерверовСозданияИнфраструктуры.DataNodes");
			Запись.МестоИспользования		= "КластерСерверов";
...
Функция ПолучитьРеквизитыИзМакета(Область) Экспорт
	...
	Для ИндСтроки = 1 По ОбластьСПараметрами.ВысотаТаблицы Цикл          
		...
		СтрокаТЗ.Описание 					= ОбластьСПараметрами.Область(ИндСтроки, 2, ИндСтроки, 2).Текст;
		СтрокаТЗ.ИмяВAPI 					= ОбластьСПараметрами.Область(ИндСтроки, 3, ИндСтроки, 3).Текст;
		СтрокаТЗ.ЗначениеПоУмолчанию 		= ОбластьСПараметрами.Область(ИндСтроки, 4, ИндСтроки, 4).Текст;
		СтрокаТЗ.Regex 						= ОбластьСПараметрами.Область(ИндСтроки, 5, ИндСтроки, 5).Текст;
		СтрокаТЗ.ИдентификаторРеквизита 	= ОбластьСПараметрами.Область(ИндСтроки, 6, ИндСтроки, 6).Текст;
		СтрокаТЗ.Отображать                 = ОбластьСПараметрами.Область(ИндСтроки, 7, ИндСтроки, 7).Текст;
		СтрокаТЗ.ОбязателенДляЗаполнения 	= ОбластьСПараметрами.Область(ИндСтроки, 8, ИндСтроки, 8).Текст;
		СтрокаТЗ.ТипЗначения 				= ОбластьСПараметрами.Область(ИндСтроки, 9, ИндСтроки, 9).Текст; 
		СтрокаТЗ.Перечисление 				= ОбластьСПараметрами.Область(ИндСтроки, 10, ИндСтроки, 10).Текст;
	КонецЦикла;
...
&НаСервере
Функция ПолучитьПеречислениеИзМакета(Область) Экспорт
	
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	Макет 			= ОбработкаОбъект.ПолучитьМакет("ПеречисленияЗначений");
	
	ОбластьСПеречислением = Макет.ПолучитьОбласть(Область);
	
	МассивПеречисления = Новый Массив;
	
	Для ИндСтроки = 1 По ОбластьСПеречислением.ВысотаТаблицы Цикл          
		МассивПеречисления.Добавить(ОбластьСПеречислением.Область(ИндСтроки, 2, ИндСтроки, 2).Текст);
	КонецЦикла;
...
Процедура ЗаполнитьТаблицуДаннымиРС(ИдентификаторЭлемента)
	...
	Пока Выборка.Следующий() Цикл	
		...
		СтрокаДерева = ДеревоРеквизитов.ПолучитьЭлементы().Добавить();
		//Элементы.ДеревоРеквизитов..СтрокаДерева.ПолучитьИдентификатор();
...
Процедура ДеревоРеквизитовЗначениеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.ИдентификаторРеквизита = "cba48746-01d2-447a-aea1-f2de5f9787f2" Тогда
		...
		Если Итератор > 0 Тогда 
			...
			ПараметрыДанных.Вставить("ГруппировкаСерверов", 			ТекущиеДанные.ГруппировкаСерверов);
...
Процедура ДобавитьНовыеРеквизитыПоДискам(ПараметрыДанных)
	...
	Для А = ПараметрыДанных.КоличествоТекущее+1 По ПараметрыДанных.КоличествоНовое Цикл 
		...
		Если ЗначениеЗаполнено(ТекущийИдентификаторЭлемента) Тогда 			
			ЗаписатьРеквизитВРС(СтрокаДерева.Реквизит, ТекущийИдентификаторЭлемента, Ложь, ПараметрыДанных.ГруппировкаСерверов, ПараметрыДанных.ВыбранныйИдентификаторРеквизита);
		Иначе
			ЗаписатьВсеВложенныеРеквизитыВРС(СтрокаДерева.Реквизит, Ложь, ПараметрыДанных.ГруппировкаСерверов, ПараметрыДанных.ВыбранныйИдентификаторРеквизита);	
		КонецЕсли;
		...
		Для каждого РеквизитДиска Из ОбщиеРеквизитыДиска Цикл
			...
			Если ЗначениеЗаполнено(ТекущийИдентификаторЭлемента) Тогда 
				ЗаписатьРеквизитВРС(СтрокаДереваРеквизит.Реквизит, ТекущийИдентификаторЭлемента, Истина, ПараметрыДанных.ГруппировкаСерверов, СтрокаДерева.ИдентификаторРеквизита);
			Иначе
				ЗаписатьВсеВложенныеРеквизитыВРС(СтрокаДереваРеквизит.Реквизит, Истина, ПараметрыДанных.ГруппировкаСерверов, СтрокаДерева.ИдентификаторРеквизита);	
			КонецЕсли;  
		КонецЦикла;
...
&НаСервере
Процедура ЗаписатьРеквизитВРС(ИмяРеквизит, ИдентификаторЭлемента, ОбязателенДляЗаполнения, ГруппировкаСерверов, Родитель)
...
&НаСервере
Процедура ЗаписатьВсеВложенныеРеквизитыВРС(ИмяРеквизит, ОбязателенДляЗаполнения, ГруппировкаСерверов, Родитель)
	...
	Для каждого Элемент Из МассивСерверов Цикл		
		ЗаписатьРеквизитВРС(ИмяРеквизит, Элемент, ОбязателенДляЗаполнения, ГруппировкаСерверов, Родитель);	
	КонецЦикла;
