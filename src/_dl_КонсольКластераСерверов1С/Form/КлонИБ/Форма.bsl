		...
		Для каждого ГруппировкаЭлементов Из МассивГруппировокЭлементов Цикл
			...
			Для каждого ИмяЭлемента Из МассивИменЭлементов Цикл
				...
				Для каждого Кубик Из МассивКубиков Цикл 
					...
					ЗаписатьРеквизитыВРС(Реквизиты, ЗначенияПоУмолчанию, Кубик);	
				КонецЦикла;
			КонецЦикла;	
		КонецЦикла;
	КонецЕсли;	
...
Процедура СоздатьИнфраструктуру()
	...
	Пока ВыборкаОбъекты.Следующий() Цикл
		...
		//VMWare    
		ДанныеКубика = ТЗВсеДанные.НайтиСтроки(Новый Структура("ИдентификаторЭлемента, ГруппировкаРеквизитов", ВыборкаОбъекты.ИдентификаторЭлемента, "VMWare"));
		...
		СтрокаЭлементыИнфраструктурыОбъектов.ИмяКубика 				= "VMWare";
		СтрокаЭлементыИнфраструктурыОбъектов.Статус 				= ПредопределенноеЗначение("Перечисление._dl_СтатусСозданияИнфраструктуры.start");
		СтрокаЭлементыИнфраструктурыОбъектов.ИдентификаторОбъекта 	= ВыборкаОбъекты.ИдентификаторЭлемента;
		...
		Если ДанныеОтвета.Свойство("task_id") Тогда 
			...
			Для Каждого Стр Из ДанныеОтвета.step_list Цикл 
				...
				НовСтр.ИдентификаторОбъекта 			= ВыборкаОбъекты.ИдентификаторЭлемента;
	...
	//SQL
	ОбработатьКубик("SQL", 2, "", ТЗВсеДанные, СпрИнфраструктура);
	
	//Кластер 1С
	ОбработатьКубик("Кластер 1С", 3, "", ТЗВсеДанные, СпрИнфраструктура);
