...
Процедура ЗаполнитьПрогресс()
	...
	ДеревоПрогресса.ПолучитьЭлементы().Очистить();
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	_dl_ЗаданиеНаСозданиеИнфраструктурыЭлементыИнфраструктурыОбъектов.ИмяКубика КАК ИмяКубика,
		|	_dl_ЗаданиеНаСозданиеИнфраструктурыЭлементыИнфраструктурыОбъектов.task_status КАК task_status,
		|	_dl_ЗаданиеНаСозданиеИнфраструктурыЭлементыИнфраструктурыОбъектов.КонфигКубика КАК КонфигКубика,
		|	_dl_ЗаданиеНаСозданиеИнфраструктурыШагиВыполненияЭлементов.step_number КАК step_number,
		|	_dl_ЗаданиеНаСозданиеИнфраструктурыШагиВыполненияЭлементов.step_name КАК step_name,
		|	_dl_ЗаданиеНаСозданиеИнфраструктурыШагиВыполненияЭлементов.step_status КАК step_status,
		|	_dl_ЗаданиеНаСозданиеИнфраструктурыЭлементыИнфраструктурыОбъектов.ИдентификаторКубика КАК ИдентификаторКубика,
		|	_dl_ЗаданиеНаСозданиеИнфраструктурыЭлементыИнфраструктурыОбъектов.Порядок КАК Порядок,
		|	_dl_ЗаданиеНаСозданиеИнфраструктурыЭлементыИнфраструктурыОбъектов.ШагСозданияИнфраструктуры КАК ШагСозданияИнфраструктуры
		|ИЗ
		|	Справочник._dl_ЗаданиеНаСозданиеИнфраструктуры.ЭлементыИнфраструктурыОбъектов КАК _dl_ЗаданиеНаСозданиеИнфраструктурыЭлементыИнфраструктурыОбъектов
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник._dl_ЗаданиеНаСозданиеИнфраструктуры.ШагиВыполненияЭлементов КАК _dl_ЗаданиеНаСозданиеИнфраструктурыШагиВыполненияЭлементов
		|		ПО _dl_ЗаданиеНаСозданиеИнфраструктурыЭлементыИнфраструктурыОбъектов.ИдентификаторКубика = _dl_ЗаданиеНаСозданиеИнфраструктурыШагиВыполненияЭлементов.ИдентификаторКубика
		|ГДЕ
		|	_dl_ЗаданиеНаСозданиеИнфраструктурыЭлементыИнфраструктурыОбъектов.Ссылка = &Ссылка
		|
		|УПОРЯДОЧИТЬ ПО
		|	Порядок,
		|	ИмяКубика
		|ИТОГИ ПО
		|	ШагСозданияИнфраструктуры,
		|	ИдентификаторКубика,
		|	ИмяКубика,
		|	КонфигКубика,
		|	task_status";
	
	Запрос.УстановитьПараметр("Ссылка", ЗаданиеНаСозданиеИнфраструктуры);
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.Уровень() = 1 Тогда
			СтрокаДереваШаг 					= ДеревоПрогресса.ПолучитьЭлементы().Добавить();
			СтрокаДереваШаг.Процедура 			= Выборка.ШагСозданияИнфраструктуры;
			СтрокаДереваШаг.ИндексКартинки		= ПолучитьИндексКартинкиПоСтатусу(Выборка.task_status);
		КонецЕсли;
		
		Если Выборка.Уровень() = 5 Тогда
			СтрокаДереваКубик 					= СтрокаДереваШаг.ПолучитьЭлементы().Добавить();
			СтрокаДереваКубик.Процедура 		= Выборка.ИмяКубика;
			СтрокаДереваКубик.СтатусПрогресса 	= Выборка.task_status;
			СтрокаДереваКубик.ИндексКартинки	= ПолучитьИндексКартинкиПоСтатусу(Выборка.task_status);
			СтрокаДереваКубик.Описание		 	= Выборка.КонфигКубика;	
		КонецЕсли; 
		
		Если Выборка.Уровень() = 6 Тогда
			СтрокаДереваДетальные 					= СтрокаДереваКубик.ПолучитьЭлементы().Добавить();
			СтрокаДереваДетальные.Процедура 		= Выборка.step_name;
			СтрокаДереваДетальные.СтатусПрогресса 	= Выборка.step_status;
			СтрокаДереваДетальные.ИндексКартинки	= ПолучитьИндексКартинкиПоСтатусу(Выборка.step_status);
			СтрокаДереваДетальные.Описание		 	= "Ошибки:";	
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Функция ПолучитьИндексКартинкиПоСтатусу(Статус)
	
	ИндексКартинки = 1;
	Если Статус = ПредопределенноеЗначение("Перечисление._dl_СтатусСозданияИнфраструктуры.fail") Тогда
		ИндексКартинки = 2;	
	ИначеЕсли Статус = ПредопределенноеЗначение("Перечисление._dl_СтатусСозданияИнфраструктуры.inproccess") Тогда
		ИндексКартинки = 1;	
	ИначеЕсли Статус = ПредопределенноеЗначение("Перечисление._dl_СтатусСозданияИнфраструктуры.start") Тогда
		ИндексКартинки = 1;	
	ИначеЕсли Статус = ПредопределенноеЗначение("Перечисление._dl_СтатусСозданияИнфраструктуры.successful") Тогда
		ИндексКартинки = 0;	
	КонецЕсли;   
...
Процедура ВосстановитьПараметры()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ГруппировкаСерверов = "VMs" 
			ИЛИ Выборка.ГруппировкаСерверов = "SQLClusters"
			ИЛИ Выборка.ГруппировкаСерверов = "1CClusters" Тогда
			
			ОдиночныеГруппировки = Истина;
		Иначе
...
Процедура Готово(Команда)  
	...
	ПодключитьОбработчикОжидания("ОбновитьСтатусы", 10, Ложь);
		...
		Пока ВыборкаИмяЭлемента.Следующий() Цикл	
			...
			Если ДанныеОтвета.Свойство("task_id") Тогда 
				...
				Для Каждого Стр Из ДанныеОтвета.step_list Цикл 
					...
					НовСтр.step_status = Перечисления._dl_СтатусСозданияИнфраструктуры[Стр.step_status];//ПредопределенноеЗначение("Перечисление._dl_СтатусСозданияИнфраструктуры.start");
